 ...
 */
 ...
 const Express = config => async specs => {
     ...
     const RegisterSpecs = config => specs => async express => {
         ...
         const AttachSwaggerDynamicContent = spec => {
            if(config.rest?.openApiFunc) Object.values(config.rest.openApiFunc).forEach(func => func(spec));
         };

         // Load Specs, Register endpoints
         const RegisterSpec = config => express => async spec => {
             AttachSwaggerDynamicContent(spec)
